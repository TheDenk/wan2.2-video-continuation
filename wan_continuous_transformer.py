...
class WanAttnProcessor:
    ...
    def __call__(
        ...
        if hidden_states_prev_len is not None:
            prev_query, noize_query = query[:, :hidden_states_prev_len].contiguous(), query[:, hidden_states_prev_len:].contiguous()
            prev_key = key[:, :hidden_states_prev_len].contiguous()
            prev_value = value[:, :hidden_states_prev_len].contiguous()

            hidden_states_prev = dispatch_attention_fn(
                prev_query,
                prev_key,
                prev_value,
            ...
            hidden_states = torch.cat([hidden_states_prev, hidden_states], dim=1).contiguous()
        else:
            hidden_states = dispatch_attention_fn(
                query,
                key,
...
class WanAttention(torch.nn.Module, AttentionModuleMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        return self.processor(self, hidden_states, encoder_hidden_states, attention_mask, rotary_emb, hidden_states_prev_len, **kwargs)
...
class WanTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        attn_output = self.attn1(norm_hidden_states, None, None, rotary_emb, hidden_states_prev_len)
        hidden_states = (hidden_states.float() + attn_output * gate_msa).type_as(hidden_states)

        # 2. Cross-attention
        norm_hidden_states = self.norm2(hidden_states.float()).type_as(hidden_states)
        attn_output = self.attn2(norm_hidden_states, encoder_hidden_states, None, None, hidden_states_prev_len)
            ...
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                for block in self.blocks:
                    hidden_states = self._gradient_checkpointing_func(
                        block, hidden_states, encoder_hidden_states, timestep_proj, rotary_emb, hidden_states_prev_len
                    )
            else:
                for block in self.blocks:
                    hidden_states = block(hidden_states, encoder_hidden_states, timestep_proj, rotary_emb, hidden_states_prev_len)
